...
ios/                     SwiftUI app (Xcode project generated by XcodeGen)
  ...
  Packages/TenpoKit/    SPM package with all core modules (§4.2)
...
```sh
cd ios
xcodegen generate          # regenerates Tenpo.xcodeproj from project.yml
open Tenpo.xcodeproj      # build & run the Tenpo scheme on an iOS 17+ simulator
```

Package tests run on macOS directly:

```sh
cd ios/Packages/TenpoKit && swift test
```
...
3. **Point the app at it.** Fill `ios/App/Config/TenpoConfig.plist`: `PROXY_URL`
...
4. **Run on your iPhone (no paid Apple account needed).** In Xcode: Settings →
   ...
   select the Tenpo target → Signing & Capabilities → choose the Personal Team.
